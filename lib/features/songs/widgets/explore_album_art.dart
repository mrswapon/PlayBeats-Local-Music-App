  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ClipOval(
        child: CustomPaint(
          size: Size(size, size),
          painter: _AlbumPainter(variant: variant),
        ),
      ),
    );
  }
...
class _AlbumPainter extends CustomPainter {
  final int variant;
  _AlbumPainter({required this.variant});
  ...
  void _paintBoldType(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF222238));
    for (var i = 0; i < 3; i++) {
      canvas.drawLine(
    ...
    _drawText(canvas, '10', Offset(r * 0.25, r * 0.4), r * 1.0,
        FontWeight.w900, Colors.white.withValues(alpha: 0.1));
  }

  void _paintWaves(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF1E1E32));
    for (var w = 0; w < 3; w++) {
      final path = Path();
      ...
      canvas.drawPath(
        ...
        Paint()
          ..style = PaintingStyle.stroke
          ..color = Colors.white.withValues(alpha: 0.1 - w * 0.02)
          ..strokeWidth = 1.2,
      );
    }
    _drawText(canvas, 'DREAMS', Offset(r * 0.35, r * 0.5), r * 0.24,
        FontWeight.w400, Colors.white.withValues(alpha: 0.15),
        fontFamily: 'monospace');
  }

  void _paintMinimalCircle(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF252540));
    canvas.drawCircle(
        c,
        r * 0.5,
        Paint()
    ...
    canvas.drawCircle(
        ...
        Paint()
          ...
          ..strokeWidth = 0.8);
    canvas.drawCircle(
        c, r * 0.1, Paint()..color = Colors.white.withValues(alpha: 0.08));
    canvas.drawLine(
  ...
  void _paintArchive(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF1C1C30));
    canvas.drawRect(
    ...
    _drawText(canvas, 'ARCHIVE', Offset(r * 0.3, r * 0.85), r * 0.26,
        FontWeight.w900, Colors.white.withValues(alpha: 0.18),
        fontFamily: 'monospace');
  }

  void _paintNoise(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF20203A));
    final rng = Random(42);
    for (var i = 0; i < 80; i++) {
      ...
      if ((Offset(dx, dy) - c).distance < r * 0.9) {
        canvas.drawCircle(
          ...
          rng.nextDouble() * 1.5,
          Paint()
            ..color = Colors.white.withValues(alpha: rng.nextDouble() * 0.06),
        );
      }
    }
    _drawText(canvas, 'C', Offset(r * 0.55, r * 0.55), r * 0.9,
        FontWeight.w700, Colors.white.withValues(alpha: 0.06),
        fontFamily: 'serif');
  }
  ...
  void _paintSplit(Canvas canvas, Size size, Offset c, double r) {
    ...
    final path = Path()
      ...
      ..close();
    canvas.drawPath(path, Paint()..color = const Color(0xFF222240));
  ...
  void _paintGeometric(Canvas canvas, Size size, Offset c, double r) {
    canvas.drawCircle(c, r, Paint()..color = const Color(0xFF24243C));
    canvas.save();
    ...
    canvas.restore();
    canvas.drawCircle(
        c, r * 0.07, Paint()..color = Colors.white.withValues(alpha: 0.1));
  }
  ...
  @override
  bool shouldRepaint(covariant _AlbumPainter old) => old.variant != variant;
}
